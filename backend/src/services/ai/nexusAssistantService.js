...
class NexusAssistantService {
  ...
  async _handleStudentChat(user, query) {
    ...
    if (q.includes('readiness') || q.includes('score') || q.includes('why is my score')) {
      return {
        reply: `Your current Career Readiness score is **${context.readinessScore}%**. Readiness is computed across 5 deterministic pillars: Skill Verification (30%), Diagnostic Assessments (25%), Project Proofs (20%), Learning Progress (15%), and Profile Completeness (10%). Submitting verified projects and taking assessments will yield the fastest score elevation.`,
