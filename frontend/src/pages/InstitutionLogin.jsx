...
export default function InstitutionLogin({ onLoginSuccess, onBackToRoles, onNavigateToOtp, onNavigateToForgot }) {
  ...
  useEffect(() => {
    if (!masterQuery.trim() && !selectedDistrict) {
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  <input
                    ...
                    style={{ accentColor: 'var(--cyber-purple)' }}
                  />
                      ...
                      <div style={{ background: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: '12px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '8px', marginBottom: '8px' }}>
                          <select
                            ...
                            style={{ background: '#0B1120', border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '8px', color: 'var(--text-primary)', fontSize: '12px' }}
                          >
                            <option value="">All 38 Districts</option>
                            {districts.map((d, i) => (
                              <option key={i} value={d.name}>{d.name} ({d.collegesCount})</option>
                            ))}
                          </select>
