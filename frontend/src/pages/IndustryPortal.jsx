...
export default function IndustryPortal({
  ...
  const institutionTabs = useMemo(() => [
    { id: 'colleges', label: 'Campus Directory', icon: Building },
    { id: 'access-requests', label: 'Student Access Requests', icon: ShieldCheck, badge: 'Active' }
  ], []);
  ...
  const renderScreenContent = () => (
    <>
      ...
      {/* SUPPORTING SCREEN: COLLEGES */}
      {activeTab === 'colleges' && (
        <CompanyColleges
          onFilterCollege={(colName) => setSearchFilterPreset({ college: colName })}
          onTabSelect={(tab) => handleTabChange(tab)}
        />
      )}
