...
export default function StudentDashboard({ setActivePage, onShowToast, user, onOpenAIModal }) {
  ...
  // Month navigation for web usage heatmap
  const [monthOffset, setMonthOffset] = useState(0);
  const currentMonthLabel = useMemo(() => {
    const d = new Date(2026, 8 + monthOffset, 1);
  ...
  const capabilities = [
    { label: 'Technical Skills', value: dashboardData.capabilities.technicalSkills, color: '#28D7FF' },
    { label: 'Problem Solving', value: dashboardData.capabilities.problemSolving, color: '#3478FF' },
    { label: 'Communication', value: dashboardData.capabilities.communication, color: '#8B5CF6' },
    { label: 'System Design', value: dashboardData.capabilities.systemDesign, color: '#FF9D4D', isGap: dashboardData.capabilities.systemDesign < 50 },
    { label: 'Cloud', value: dashboardData.capabilities.cloud, color: '#FF9D4D', isGap: dashboardData.capabilities.cloud < 50 }
  ];

  /* ---- Top Opportunities (real backend matches) ---- */
  ...
  /* ---- Activity Heatmap (derived from real student activity timestamps) ---- */
  const heatmapData = useMemo(() => {
    if (Array.isArray(dashboardData.heatmapData) && dashboardData.heatmapData.length === 7) {
      return dashboardData.heatmapData;
    }
    return Array.from({ length: 7 }, () => new Array(30).fill(0));
  }, [dashboardData.heatmapData]);

  const heatmapColors = ['#0d1b2a', '#0d2847', '#1a5276', '#2580b8', '#28D7FF'];
  ...
  const passportTechs = (dashboardData.passport.technologies || []).map(tech => {
    const key = String(tech.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    return {
      name: tech.name,
      color: TECH_COLORS[key] || '#8B5CF6',
      icon: TECH_ICONS[key] || String(tech.name || '?').slice(0, 2)
    };
  });
    ...
    <div style={{ padding: '20px 24px 40px', maxWidth: '1400px', width: '100%', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: '28px', fontWeight: 900, color: 'var(--cyber-cyan)', fontFamily: 'var(--font-mono)' }}>
                {liveStats.careerJourney || 82}%
              </div>
            </div>
            <div style={{ position: 'relative', width: '64px', height: '64px' }}>
              <CircularProgress size={64} strokeWidth={6} progress={liveStats.careerJourney || 82} color="var(--cyber-cyan)" />
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '16px', marginBottom: '20px' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg, #7C3AED, #0EA5E9)', opacity: 0.8 }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-heading)' }}>
              <Sparkles size={15} color="#7C3AED" style={{ verticalAlign: 'middle', marginRight: '6px' }} />
              NEXUS AI Recommendation
            </span>
            <span style={{ ...s.badge, background: 'var(--cyber-purple-dim)', color: '#7C3AED', border: '1px solid rgba(124, 58, 237, 0.25)' }}>
              Personalized
            </span>
          </div>
          ...
          {dashboardData.topOpportunities.length > 0 ? (
            <div>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: 500 }}>
                Matched Company Opportunity • {dashboardData.topOpportunities[0].match}% Fit
              </div>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: 'var(--text-heading)', marginBottom: '6px' }}>
                {dashboardData.topOpportunities[0].title}
              </h3>
              <div style={{ fontSize: '12px', color: 'var(--cyber-cyan)', fontWeight: 600, marginBottom: '10px' }}>
                {dashboardData.topOpportunities[0].company}
              </div>

              {/* Explainable Reasons */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '14px' }}>
                {(dashboardData.topOpportunities[0].reasons && dashboardData.topOpportunities[0].reasons.length > 0
                  ? dashboardData.topOpportunities[0].reasons.slice(0, 2)
                  : ['Complements verified profile skills', 'Strong industry hiring velocity']
                ).map((reason, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11.5px', color: 'var(--text-secondary)', fontWeight: 600 }}>
                    <CheckCircle2 size={13} color="#10B981" />
                    <span>{reason}</span>
                  </div>
                ))}
                {(dashboardData.topOpportunities[0].improvements && dashboardData.topOpportunities[0].improvements.length > 0) && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#FF9D4D', fontWeight: 600 }}>
                    <TrendingUp size={12} color="#FF9D4D" />
                    <span>{dashboardData.topOpportunities[0].improvements[0]}</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  style={{
                    ...s.ctaBtn, flex: 1, justifyContent: 'center',
                ...
                <button
                  onClick={() => openGoogleResearch(dashboardData.topOpportunities[0].title, dashboardData.topOpportunities[0].company)}
                  title="Research opportunity and company on Google"
                  style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
        ...
        <div style={{ ...s.card }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '16px' }}>
            {[
              { value: String(dashboardData.passport.verifiedSkills).padStart(2, '0'), label: 'Verified Skills', color: '#28D7FF' },
              { value: String(dashboardData.passport.projectsShipped).padStart(2, '0'), label: 'Projects Shipped', color: '#2FE0A1' },
              { value: String(dashboardData.passport.industryReviews).padStart(2, '0'), label: 'Industry Review', color: '#FF9D4D' },
              { value: String(dashboardData.passport.certifications).padStart(2, '0'), label: 'Certifications', color: '#8B5CF6' }
            ].map((ps, i) => (
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {passportTechs.map((tech, i) => (
              <div key={i} style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: tech.icon.length <= 2 ? '11px' : '16px',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2.2fr 0.8fr 1.2fr 1.3fr', gap: '16px' }}>
        ...
        {/* --- Monthly Web Usage Heatmap --- */}
        <div style={{ ...s.card }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div>
              <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>Monthly Web Usage</span>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Your activity across SkillNexus</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button 
          ...
          <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
                ...
                <div key={ri} style={{ display: 'flex', gap: '3px', marginBottom: '3px' }}>
                  {row.map((val, ci) => (
                    <div key={ci} style={{
                      ...
                      borderRadius: '3px', background: heatmapColors[val],
                      border: val > 0 ? `1px solid ${heatmapColors[val]}40` : '1px solid rgba(255,255,255,0.03)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', paddingTop: '16px', marginLeft: '12px', paddingLeft: '8px', borderLeft: '1px solid rgba(255,255,255,0.04)' }}>
              {[
                { label: 'More active', level: 4 },
                { label: 'Active', level: 3 },
                { label: 'Some activity', level: 2 },
        ...
        <div style={{ ...s.card, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                { icon: Award, value: String(dashboardData.monthlyStats.assessmentsAttempted), label: 'Assessments attempted', color: '#8B5CF6' },
                { icon: BookOpen, value: String(dashboardData.coursesEnrolled), label: 'Courses enrolled', color: '#2FE0A1' },
                { icon: ShieldCheck, value: String(dashboardData.skillsVerified), label: 'Skills verified', color: '#28D7FF' },
                { icon: Briefcase, value: String(dashboardData.opportunitiesApplied), label: 'Applications sent', color: '#FF9D4D' }
              ].map((stat, i) => {
        ...
        onCompleted={(result) => {
          ...
          if (onShowToast) {
            onShowToast({
              ...
              message: `Baseline readiness score computed: ${result?.readinessScore || 75}%. Matched opportunities unlocked.`,
              type: 'success'
            });
          }
        }}
